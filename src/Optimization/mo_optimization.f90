...
contains
  ...
  subroutine optimization()
    ...
    !all opti function options adapted from mHM implementations
    use mo_objective_function,            only: objective!,loglikelihood,loglikelihood_stddev
    use mo_objective_function,            only: opti_eval
    use mo_wqm_global_variables,          only: wqm_param,nTotalParam,dir_Out
    use mo_wqm_constants,                 only: maxngrps
    use mo_wqm_write,                     only :wqm_write,write_nml_par
    ...
    integer(i4)                             :: npara,ip,igrp,it
    real(dp), allocatable                   :: local_parameters(:,:),ini_parameters(:,:) ! global_parameters but includes a and b for likelihood
    ...
    ! START
    ! -------------------------------------------------------------------------
    !convert the new parameter variable structure to that of mHM
    !And store in "global_parameters"
    allocate(ini_parameters(maxngrps*nTotalParam,4))
    it = 0_i4
    do ip=1,nTotalParam    
      do igrp = 1, wqm_param(ip)%ngroups
          it = it+1_i4
          ini_parameters(it,1) = wqm_param(ip)%lowbound(igrp)  !parameter lowbound
          ini_parameters(it,2) = wqm_param(ip)%upbound(igrp)  !parameter upbound          
          ini_parameters(it,3) = wqm_param(ip)%pvalue(igrp)  !parameter value
          ini_parameters(it,4) = wqm_param(ip)%flag(igrp)  !opti flag 1 true, 0 false
      end do
    end do
    if (.not. allocated(global_parameters)) allocate(global_parameters(it,4))
    global_parameters(:,:) = ini_parameters(1:it,:)
    ...
    case (0)
       ...
       ! tFile = trim(adjustl(dir_Out)) // 'mcmc_tmp_parasets.nc'
    ...
    case (1)
       ...
       tFile = trim(adjustl(dir_Out)) // 'dds_results.out'
    ...
    ! case (2)
       ...
       ! tFile = trim(adjustl(dir_Out)) // 'anneal_results.out'
    ...
    ! case (3)
       ...
       ! tFile = trim(adjustl(dir_Out)) // 'sce_results.out'
       ! pFile =  trim(adjustl(dir_Out)) // 'sce_population.out'
       ...
       ! local_parameters(:,3) = sce(objective, local_parameters(:,3), local_parameters(:,1:2),                              &
            ...
            ! bestf=funcbest)
    case default
       call finish('This optimization method is not implemented.')
    end select
    ...
    call message('')
    call message('Final run using the optimized parameter set')
    !run once more for the final optimal parameter set
    call opti_eval(global_parameters(:,3))  
    
    ! ---------------------------------------------------------------------------  
    ! WRITE Discharge and Nutrients at gauging locations
    ! ---------------------------------------------------------------------------
    call wqm_write()
    !convert back to wqm_param structure with updated values
    it = 0_i4
    do ip=1,nTotalParam    
      do igrp = 1, wqm_param(ip)%ngroups
          it = it+1_i4
          wqm_param(ip)%pvalue(igrp) = global_parameters(it,3)!parameter value
      end do
    end do
    call write_nml_par()
